...
def desafio007():
  ...
  return 'A média entre {:.1f} e {:.1f} é igual a {:.1f}'.format(n1, n2, m)
...
def desafio008():
  ...
  return f'a média  de  {média}m coresponde a \n {km}km \n {hm}hm \n {dam}dam \n {dm}dm \n {cm}cm \n {mm}mm'
...
def desafio010():
  ...
  return 'Com R${:.2f} você pode comprar US${:.2f} e também €{:.2f}'.format(real, dolar, euro)
...
def desafio012():
  ...
  return 'O produto que custava R${:.2f}, na promoção de 5% vai custa R${:.2f}'.format(preço, novo)
...
def desafio015():
  ...
  return 'O total a pagar é de R${:.2f}'.format(pago)



def desafio016():
  num = float(input('Digite um valor: '))
  return 'O valor digitado foi de {} e sua porção inteira é {}'.format(num, int(num))
...
def desafio019():
  ...
  return f'O aluno escolhido foi {escolhido}'
...
def desafio024():
  ...
  return cid[:5].upper() == 'SANTO'



def desafio025():
  nome = input('Qual seu nome completo? ').strip()
  return f'Seu nome tem Silva? {"silva" in nome.lower()}'
...
def desafio034():
  ...
  return f'Quem ganhava R${salário} passa a ganhar R$ {novo} agora.'
